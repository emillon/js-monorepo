  ...
  let (_the_group : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.group) =
    {
      ...
      origin =
        "ppx_sexp_conv.v0.14.3/test/expect/test_test.ml.Simple_grammar"
    } in
  let (t_sexp_grammar : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.t) =
    Ref ("t", _the_group) in
  t_sexp_grammar
  ...
  let (_the_group : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.group) =
    {
      ...
      generic_group = _the_generic_group;
      origin =
        "ppx_sexp_conv.v0.14.3/test/expect/test_test.ml.Recursive_group"
    } in
  let (t_sexp_grammar : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.t) =
    Ref ("t", _the_group)
      ...
      types =
        [("t",
           (Explicit_bind
              (["a"; "b"],
                (Grammar
                   Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.fun_sexp_grammar))))]
    } in
  let (_the_group : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.group) =
    {
      gid = (Ppx_sexp_conv_lib.Lazy_group_id.create ());
      apply_implicit = [];
      generic_group = _the_generic_group;
      origin = "ppx_sexp_conv.v0.14.3/test/expect/test_test.ml.Functions"
    } in
  let (t_sexp_grammar : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.t) =
    Ref ("t", _the_group) in
  t_sexp_grammar
