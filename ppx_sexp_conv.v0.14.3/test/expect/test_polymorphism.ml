    ...
    Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.generic_group) =
    {
      ...
      types =
        [("t",
           (Explicit_bind
              (["a"; "_"; "b"],
                (List [One (Explicit_var 0); One (Explicit_var 2)]))));
        ("u",
          (Apply
             ((Recursive "t"),
               [Implicit_var 0; Implicit_var 1; Implicit_var 2])))]
    } in
  let (_the_group : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.group) =
    {
      gid = (Ppx_sexp_conv_lib.Lazy_group_id.create ());
      apply_implicit =
        [string_sexp_grammar; int_sexp_grammar; float_sexp_grammar];
      generic_group = _the_generic_group;
      origin = "ppx_sexp_conv.v0.14.3/test/expect/test_polymorphism.ml"
    } in
  let (t_sexp_grammar : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.t) =
    Ref ("t", _the_group)
