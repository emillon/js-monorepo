  ...
  let (_the_generic_group :
    Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.generic_group) =
    {
      ...
      types =
        [("t",
           (Explicit_bind
              (["a"], (Apply ((Implicit_var 0), [Explicit_var 0])))))]
    } in
  let (_the_group : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.group) =
    {
      ...
      origin = "ppx_sexp_conv.v0.14.3/test/expect/test_functors.ml.Maybe"
    } in
  let (t_sexp_grammar : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.t) =
    Ref ("t", _the_group) in
  t_sexp_grammar
  ...
  let (_the_group : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.group) =
    {
      ...
      generic_group = _the_generic_group;
      origin = "ppx_sexp_conv.v0.14.3/test/expect/test_functors.ml.Make"
    } in
  let (t_sexp_grammar : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.t) =
    Ref ("t", _the_group)
                ...
                (Variant
                   {
                     ...
                     alts =
                       [("V",
                          [One (Apply ((Implicit_var 0), [Explicit_var 0]))])]
                   }))))]
    } in
  let (_the_group : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.group) =
    {
  ...
  let (_the_group : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.group) =
    {
      gid = (Ppx_sexp_conv_lib.Lazy_group_id.create ());
      apply_implicit =
        [int_sexp_grammar; T2.t_sexp_grammar; T1.t_sexp_grammar];
      generic_group = _the_generic_group;
      origin = "ppx_sexp_conv.v0.14.3/test/expect/test_functors.ml"
    } in
  let (t_sexp_grammar : Ppx_sexp_conv_lib.Sexp.Private.Raw_grammar.t) =
    Ref ("t", _the_group) in
  t_sexp_grammar
