...
let parse_eager_cst ~no_sexp_is_error str =
  let r = ref [] in
  let state = Eager_cst.State.create ~no_sexp_is_error (fun _ v -> r := v :: !r) in
  Result.try_with (fun () ->
    let stack = Eager_cst.Stack.empty in
    let stack = Eager_cst.feed_string state str stack in
    Eager_cst.feed_eoi state stack;
    List.rev !r)
;;

let parse_eager_cst_no_sexp_is_error = parse_eager_cst ~no_sexp_is_error:true
let parse_eager_cst = parse_eager_cst ~no_sexp_is_error:false
...
let%expect_test "regression test (we didn't run the callback on comments, resulting in \
  ...
  =
  let test s =
    parse_eager_cst s |> [%sexp_of: (Cst.t_or_comment list, exn) Result.t] |> print_s
  in
  test ";";
  [%expect
    {|
...
let%expect_test "eager parser semantics" =
  ...
  let got_sexp state sexp =
    Exn.raise_without_backtrace
      (Got_sexp { sexp; offset = P.State.Read_only.offset state })
  in
  ...
  List.iter inputs ~f:(fun input ->
    let rec go start state stack i =
      ...
      then (
        ...
        match P.feed state input.[i] stack with
        ...
        | exception exn -> Error (i, exn)
        | stack -> go start state stack (i + 1))
    in
    let state = P.State.create got_sexp in
    match go 0 state P.Stack.empty 0 with
    | Ok () -> ()
    | Error exn ->
      (match Many.parse_string input with
       | Ok result_from_many ->
         print_s
           [%message
