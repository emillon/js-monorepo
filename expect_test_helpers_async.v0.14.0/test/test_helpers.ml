...
let%expect_test "[run ~hide_positions:true]" =
  ...
  in
  [%expect {|
    expect_test_helpers_async.v0.14.0/test/test_helpers.ml:LINE:COL |}]
;;
...
let%expect_test "run, with postprocess" =
  ...
  in
  [%expect {|
    expect_foo_helpers_async.v0.14.0/foo/foo_helpers.ml:LINE:COL |}]
;;
...
let%expect_test "system, with non-existent command" =
  with_cd_into_temp_dir (fun () ->
    ...
    [%expect
      ...
      --- STDERR ---
      /bin/bash: line 1: ./i-hope-this-does-not-exist: No such file or directory |}])
;;
...
let%expect_test "[show_raise_async], raises hiding positions" =
  ...
  [%expect
    {|
    (raised expect_test_helpers_async.v0.14.0/test/test_helpers.ml:LINE:COL) |}]
;;
...
let%expect_test "[require_does_not_raise_async], raises" =
  ...
  let%bind () =
    [%expect
      {|
    (* require-failed: expect_test_helpers_async.v0.14.0/test/test_helpers.ml:LINE:COL. *)
